...
\paragraph{Deployment på MySQL server}
...
Forsøget med MySQL blev foretaget på en Virtuel Private Server (VPS), med domænet joachim.io. Herpå blev mysql installeret, og der blev lavet en IPTABLES\footnote{IPtables er et netværksadministrationsprogram til Linux} rule der tillod adgang til databasen gennem port 3306. For at der kan forbindes til MySQL server skal der til dels oprettes en MySQL user på databasen, der har rettigheder til at connecte. Disse users blev oprettet manuelt ved at udføre SQL statements på mysql’s User database. User skal da have rettigheder så denne bruger kan forbinde udefra og ikke kun fra localhost. Heruderover skal det specificeres i mysql’s config fil, hvilke IP addresser der har lov til at oprette forbindelse. Dette gjordes ved at sætte mysql til at lytte på  interfaces: localhost og 0.0.0.0
Problemer med Visual Studio og MySQL
Der var problemer med at få forbindelse til databasen gennem visual studio. Dette skyldes en mangel på drivere, idet MS visual strudio ikke pr. standard har funktionalitet hertil. MySQL for visual studio blev downloadet fra http://dev.mysql.com/downloads/installer/ .  Efter en mindre kamp med drivers, kunne der oprettes forbindelse til databasen gennem visual studio. Se figur XXX.
